...
Ajustado para despliegue en Render.com (producción) + desarrollo local.
"""
...
ON_RENDER = config('ON_RENDER', default=False, cast=bool)

# Usar PostgreSQL en Render si DATABASE_URL está definida
if ON_RENDER or config('DATABASE_URL', default=''):
...
# Media files (advertencia: no persisten en Render)
MEDIA_URL = '/media/'
...
# Logging: usar consola en Render, archivo en local
LOGGING = {
    'version': 1,
    ...
    'loggers': {
        'casos': {'handlers': ['console'], 'level': 'INFO', 'propagate': True},
        'usuarios': {'handlers': ['console'], 'level': 'INFO', 'propagate': True},
    },
}

# Solo en desarrollo: usar archivo de log
if not ON_RENDER:
    ...
    LOGGING['loggers']['casos']['handlers'] = ['file']
    LOGGING['loggers']['usuarios']['handlers'] = ['file']
